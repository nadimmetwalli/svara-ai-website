...
import { Play, Pause, ArrowUpRight, Phone, Clock, Globe, TrendingUp } from "lucide-react";
import { Link } from "react-router-dom";
import demoRecording from "@/assets/demo-call-recording.mp3";
import { useLanguage } from "@/i18n/LanguageContext";

const highlights = [
  { icon: Phone, label: "24/7 call handling", desc: "Never miss a call again" },
  { icon: Globe, label: "Multilingual", desc: "Speaks your customers' language" },
  { icon: Clock, label: "Instant setup", desc: "Live in under 48 hours" },
  { icon: TrendingUp, label: "Revenue recovery", desc: "Turn missed calls into orders" },
];
  ...
  return (
    <section id="demo" className="py-12 sm:py-20 px-4 overflow-hidden">
      <div className="container mx-auto max-w-5xl">
        <div className="bg-card rounded-3xl border border-border p-4 sm:p-8 md:p-12 overflow-hidden">
          <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-start">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-lg text-muted-foreground mb-8 max-w-md">
                {t.tryDemo.liveDemoDesc}
              </p>

              <div className="grid grid-cols-2 gap-4">
                {highlights.map((h, i) => (
                  <div key={i} className="flex flex-col gap-2 rounded-2xl border border-border bg-background p-4">
                    <h.icon className="w-5 h-5 text-primary" />
                    <p className="text-sm font-bold text-foreground leading-tight">{h.label}</p>
            ...
            <div className="bg-gradient-to-br from-primary to-primary/80 rounded-2xl p-4 sm:p-6 md:p-8 flex flex-col justify-between h-full">
